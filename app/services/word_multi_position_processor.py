...
class WordMultiPositionProcessor:
            ...
            for paragraph in cell.paragraphs:
                ...
                for placeholder, value in position_data.items():
                    if placeholder in paragraph_text:
                        ...
                        paragraph_text = paragraph_text.replace(placeholder, value)
                
                # Проверяем, были ли изменения
                if paragraph_text != original_text:
                    # Сохраняем форматирование первого run, если он есть
                    if paragraph.runs:
    ...
    def _replace_placeholder_in_paragraph(self, paragraph, placeholder: str, value: str) -> bool:
        """
        Заменяет плейсхолдер в параграфе, работая с runs для правильной обработки разбитых плейсхолдеров.
        
        Args:
            paragraph: Параграф для замены
            placeholder: Плейсхолдер для замены (например, "{{ payment_terms }}")
            value: Значение для замены
            
        Returns:
            True, если замена была выполнена, False иначе
        """
        # Получаем полный текст параграфа
    ...
    def _replace_placeholder_in_cell(self, cell, placeholder: str, value: str) -> bool:
        """
        Заменяет плейсхолдер в ячейке таблицы, работая с параграфами внутри ячейки.
        
        Args:
            cell: Ячейка для замены
            placeholder: Плейсхолдер для замены (например, "{{ payment_terms }}")
            value: Значение для замены
        ...
        replaced = False
        for paragraph in cell.paragraphs:
            if self._replace_placeholder_in_paragraph(paragraph, placeholder, value):
                replaced = True
        return replaced
        ...
        for table in doc.tables:
            if table != positions_table:  # Пропускаем таблицу с позициями
                for row in table.rows:
                    for cell in row.cells:
                        for key, value in word_data.items():
                            self._replace_placeholder_in_cell(cell, key, value)
            else:
        ...
        for row_idx in range(updated_total_row_start, len(positions_table.rows)):
            ...
            for cell in row.cells:
                ...
                for paragraph in cell.paragraphs:
                    ...
                    if '{{ general_prise }}' in paragraph_text and general_price is not None:
                        # В футере заменяем на общую цену всех позиций с форматированием
                        paragraph_text = paragraph_text.replace('{{ general_prise }}', self.format_price(general_price))
                    if '{{ final_price_NDS }}' in paragraph_text and final_price_nds is not None:
                        paragraph_text = paragraph_text.replace('{{ final_price_NDS }}', self.format_price(final_price_nds))
                    
                    # Обновляем текст параграфа, если были изменения
                    if paragraph_text != original_text:
