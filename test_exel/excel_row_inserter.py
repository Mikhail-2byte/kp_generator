...
def update_summary_block(sheet, rows_added: int = 0) -> None:
    ...
    formula_row = i37_row + rows_added
    base_reference = 24 + rows_added
    first_offset = 15 + rows_added
    second_offset = 16 + rows_added

    for row in range(i37_row, formula_row):
        sheet[f"I{row}"] = f"=AC{total_row}"

    sheet[f"I{formula_row}"] = (
        f"=IF(H{formula_row}=\"ДА\",I{base_reference}*3%/365*(I{first_offset}+I{second_offset}),0)"
    )

    difference_row = formula_row + 1
...
def prompt_rows_to_add() -> int:
    while True:
        raw_value = input(
            f"Введите количество строк для добавления (по умолчанию {DEFAULT_ROWS_TO_ADD}): "
        ).strip()
